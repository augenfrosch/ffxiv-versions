...
pub async fn check_versions_update_notices(versions: Versions, file_name: &str) -> Result<()> {
	...
	for version in versions {
		if let Some(update_notice_url) = &version.update_notice_url {
			...
			let update_notice_info = match file_name {
				"global" => {
					Some(global::parse_update_notice(&response_text, client.clone()).await?)
				},
				"cn" => Some(cn::parse_update_notice(&response_text)?),
				"kr" => Some(kr::parse_update_notice(&response_text)?),
				"tw" => Some(tw::parse_update_notice(&response_text)?),
